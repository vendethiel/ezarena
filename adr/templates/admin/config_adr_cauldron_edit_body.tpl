<form method="post" action="{S_CAULDRON_ACTION}">

<h1>{L_CAULDRON_TITLE}</h1>

<p>{L_CAULDRON_EXPLAIN}</p>

<table class="forumline" cellspacing="1" cellpadding="4" border="0" align="center" width="100%">
	<tr>
		<td class="row2" width="100%" align="center" ><span class="gen">{L_ITEM1_TITLE} : {ITEM1}<br \><br \>{L_ITEM2_TITLE} : {ITEM2}<br \><br \>{L_ITEM3_TITLE} : {ITEM3}<br \><br \>{L_ITEM_CREATED_TITLE} : {ITEM_CREATED}</span></td>
	</tr>
</table>

<br clear="all" />

<table class="forumline" cellspacing="1" cellpadding="4" border="0" align="center" width="95%">
	<tr>
		<td class="catBottom" align="center" colspan="2">{S_HIDDEN_FIELDS}<input class="mainoption" type="submit" value="{L_SUBMIT}" /></td>
	</tr>
</table>

</form>