...
//-- fin mod : rank color system -----------------------------------------------
	...
	{
		if ( $privmsg['user_colortext'] != '' )
		{
			$private_message = '<font color="' . $privmsg['user_colortext'] . '">' . $private_message . '</font>';
		}
	}	
		...
		else if ( $mode == 'reply' || $mode == 'quote' )
		{
			...
			if ( $mode == 'quote' )
			{
				...
				if ( $board_config['allow_colortext'] && $privmsg['user_colortext'] != '')
				{
					...
					$privmsg_message = preg_replace('/\[quote/', '[/color][quote', $privmsg_message, 1);

					$pos = false;
					if (is_int(strpos($privmsg_message, '[/quote]')))
					{
						$endPos = strlen($privmsg_message);
						while ($endPos > 0)
						{
							$endPos = $endPos - 1;
							$pos = strpos($privmsg_message, '[/quote]', $endPos);
							if (is_int($pos))
							{
								break;
							}
...
//-- fin mod : bbcode box reloaded ---------------------------------------------	
	...
	if ( $preview && !$error )
	{
		...
		if ( $board_config['allow_colortext'] )
		{
			if ( $userdata['user_colortext'] != '' )
			{
				$preview_message = '<font color="' . $userdata['user_colortext'] . '">' . $preview_message . '</font>';
			}
		}		
