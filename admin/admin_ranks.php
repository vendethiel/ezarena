	...
	else if( $mode == "save" )
	{
		...
		if ($rank_id)
		{
			...
			$sql = "UPDATE " . RANKS_TABLE . "
				SET rank_title = '" . str_replace("\'", "''", $rank_title) . "', rank_tags = '" . str_replace("\'", "''", $rank_tags) . "', rank_special = $special_rank, rank_min = $min_posts, rank_image = '" . str_replace("\'", "''", $rank_image) . "'
				WHERE rank_id = $rank_id";
		...
		{
			$sql = "INSERT INTO " . RANKS_TABLE . " (rank_title, rank_tags, rank_special, rank_min, rank_image)
				VALUES ('" . str_replace("\'", "''", $rank_title) . "', '" . str_replace("\'", "''", $rank_tags)
 . "', $special_rank, $min_posts, '" . str_replace("\'", "''", $rank_image) . "')";
...
for($i = 0; $i < $rank_count; $i++)
{
	...
	$template->assign_block_vars("ranks", array(
		"ROW_COLOR" => "#" . $row_color,
		"ROW_CLASS" => $row_class,
		"RANK" => ( sizeof ( $rank_tags ) ) ? $rank_tags[0] . $rank . $rank_tags[1] : $rank,
		"SPECIAL_RANK" => $rank_is_special,
		"RANK_MIN" => $rank_min,
