...
function init_display_template($template_var, $replacement, $filename = 'viewtopic_attach_body.tpl')
{
	...
	if (!isset($template->uncompiled_code[$template_var]) && empty($template->uncompiled_code[$template_var]))
	{
		...
		{
			die("Attach-mod Template->loadfile(): No file specified for handle $template_var");
		}
		...
		if (empty($str))
		{
			die("Attach-mod Template->loadfile(): File $filename_2 for handle $template_var is empty");
		}
	...
	{
		// V: god, why isn't that crap using $template ?
		if (file_exists($new_filename = str_replace($template->tpl, $template->tpldef, $complete_filename)))
		{
			$complete_filename = $new_filename;
		}
		else
		{
			die("Attach-mod Template->make_filename(): Error - file $complete_filename does not exist");
		}
	}
